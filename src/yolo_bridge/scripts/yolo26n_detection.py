...
class YOLO26nDetector:
    def __init__(self, param):
        ...
        if self.DetectMode in (1, 3):
            self.center_pub = rospy.Publisher("/yolo26n/target_center", PointStamped, queue_size=1)
        else:
            self.center_pub = rospy.Publisher("/yolo26n/target_bbox", BoundingBox, queue_size=1)
...
if __name__ == "__main__":
    ...
    try:
        node = YOLO26nDetector(param = args)
        node.run()
        # rospy.spin()
