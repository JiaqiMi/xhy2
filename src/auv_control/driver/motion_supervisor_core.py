...
DEFAULT_PARAMETERS = {
    ...
    'thruster_recovery_reverse_step_seconds': 0.5,
    'thruster_recovery_reverse_max_seconds': 5.0,
...
class MotionSupervisorCore(object):
    ...
    def __init__(self, parameters=None):
        ...
        self.thruster_recovery_reverse_duration = self.parameters[
    ...
    def _validate_parameters(self):
        ...
        positive_names = (
            ...
            'capture_radius', 'control_center_hold_tolerance',
            ...
            'thruster_recovery_reverse_seconds',
            'thruster_recovery_reverse_step_seconds',
            'thruster_recovery_reverse_max_seconds',
        ...
        if self.parameters['thruster_recovery_reverse_seconds'] > (
                self.parameters['thruster_recovery_reverse_max_seconds']):
            raise ValueError(
                'thruster_recovery_reverse_seconds 不能大于最大反向时长')
        for name in (
                'max_tx_positive', 'max_tx_negative',
                'max_ty_positive', 'max_ty_negative',
                'max_mz_positive', 'max_mz_negative'):
    ...
    def _hover_exit_status(self, vehicle):
        """统一计算 mode=2 与 mode=4 的 HOVER 退出滞回状态。"""
        unused_dx, unused_dy, distance, yaw_error = self._goal_metrics(vehicle)
        del unused_dx, unused_dy
        requested = (
            distance > self.parameters['hover_exit_position_error']
            or abs(yaw_error) > self.parameters['hover_exit_yaw_error']
        )
        if requested:
        ...
        triggered = (
            requested
            and wait_seconds >= self.parameters['hover_exit_hold_seconds']
        )
        diagnostics = {
            'hover_exit_requested': requested,
            'hover_exit_wait_s': wait_seconds,
            'hover_exit_triggered': triggered,
        }
    ...
    def _thruster_diagnostics(self, vehicle, tx, ty, mz):
        ...
        return {
            ...
            'thruster_recovery_reverse_duration_s': (
                self.thruster_recovery_reverse_duration),
    ...
    def _start_thruster_recovery(self, vehicle, output):
        ...
        self.thruster_recovery_reverse_duration = min(
            self.parameters['thruster_recovery_reverse_max_seconds'],
            self.parameters['thruster_recovery_reverse_seconds']
            + (self.thruster_recovery_count - 1)
            * self.parameters['thruster_recovery_reverse_step_seconds'],
        )
        ...
        self._transition(
            THRUSTER_RECOVERY,
            'TY/MZ 大指令、低功率且无运动，第 {} 次侧推自动恢复，反向 {:.1f} 秒'.format(
                self.thruster_recovery_count,
                self.thruster_recovery_reverse_duration,
            ),
        )
    ...
    def _thruster_recovery_output(self, vehicle):
        ...
        if phase == RECOVERY_REVERSE_PULSE:
            ...
            if (not success
                    and elapsed < self.thruster_recovery_reverse_duration):
                return output
    ...
    def step(self, vehicle):
        ...
        if self.state == HOVER and self.parameters['pure_depth_control']:
            ...
            hover_exit_triggered, hover_exit_diagnostics = (
                self._hover_exit_status(vehicle))
            if hover_exit_triggered:
                self._transition(
                    TRANSLATE,
                    'HOVER 误差持续超限，恢复统一三轴跟踪',
                )
                # mode=2 下控制器始终运行，不重置参考速度、制动锁存或
                # 上一周期输出，保证 HOVER 退出时力矩连续。
                output = self._unified_pose_output(vehicle)
                output.diagnostics.update(hover_exit_diagnostics)
                return output
        ...
        if self.state == CAPTURE:
            ...
            if self.handover_started_at is not None:
                # 接管确认期间也使用与 mode=2 相同的退出阈值和持续时间，
                # 避免独立 capture_exit_radius 产生不同状态语义。
                hover_exit_triggered, hover_exit_diagnostics = (
                    self._hover_exit_status(vehicle))
                if hover_exit_triggered:
                    self._reset_motion_references(vehicle.now)
                ...
                if mode_acknowledged:
                    ...
                    return self._output(
                        vehicle, MODE_DPROV, immediate_zero=True,
                        diagnostics=hover_exit_diagnostics)
                if (
        ...
        if self.state == HOVER:
            ...
            hover_exit_triggered, hover_exit_diagnostics = (
                self._hover_exit_status(vehicle))
            if hover_exit_triggered:
                # mode=4 保持期间上位机参考轨迹暂停，退出接管前以当前实测
                # 状态重建参考，随后恢复 mode=2 统一三轴跟踪。
                self._reset_motion_references(vehicle.now)
                self._transition(
                    TRANSLATE,
                    'HOVER 误差持续超限，退出接管并恢复统一三轴跟踪',
                )
                output = self._unified_pose_output(vehicle)
                output.diagnostics.update(hover_exit_diagnostics)
                return output
