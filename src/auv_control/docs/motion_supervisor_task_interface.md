...
`/cmd/motion/goal` 中的位姿始终表示最终 `base_link` 位姿。控制器内部使用
水平旋转中心 `control_link`，并根据最终 yaw 自动完成两者的刚体换算。
...
节点启动后会锁定第一帧完整有效的 `control_link` 位姿，先在该处刹停并完成
`mode=4` 接管。启动定点完成前收到的任务目标不会改变锁定位姿，只缓存最新
一条；达到配置的稳定发布帧数前，对外保持 `CAPTURE`，不会提前发布
`HOVER`。`startup_complete=true` 后自动执行缓存目标。
...
| `position_error` | m | `control_link` 到内部控制目标的水平误差 |
...
| `x/y_axis_error` | m | `control_link` 本体前、右方向控制误差 |
...
| `TRANSLATE` | 3 | X/Y 至少一轴处于 TRACK，另一轴可独立 BRAKE/HOLD |
| `TRANSLATE_BRAKE` | 4 | X/Y 均不再 TRACK，正在独立刹停或保持 |
