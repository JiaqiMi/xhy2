...
当前 `base_link` 与 IMU/GNSS 定位点重合。状态机按目标 yaw 相对当前 yaw
的最短归一化误差选择旋转中心：正误差使用正向标定结果，负误差使用负向
标定结果。进入 `ALIGN_FINAL` 后，本次方向和中心保持锁存；`FINAL_BRAKE`
中主动刹车 `MZ` 反号不会切换中心。若标定中心与 IMU 存在杆臂，
`base_link` 与 IMU 会按刚体关系沿小圆弧运动。控制中心若真实漂出
`capture_exit_radius`，状态机会先重新平移，再继续最终转向；转向结束时
`base_link` 到达任务指定的最终位姿。

任务节点只需发布最终 yaw，不要根据本周期 `MZ` 正负修改目标，也不要自行
选择正负中心。5 Hz 重复发布同一目标不会改变最终转向和刹转阶段已锁存的
计划方向。
