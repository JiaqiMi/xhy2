...
class Search2:
    ...
    def target_detection_callback(self, msg: TargetDetection):
        ...
        存的时候就应该存减去夹爪之后的位置
        """
        rospy.loginfo(f"{NODE_NAME}: 收到目标检测消息")
            ...
            with self.lock:
                ...
                if self.xyz_distance(point_in_camera, origin_in_camera) < 5.0:
                    try:
                        ...
                        target_in_base = self.tf_listener.transformPose("base_link", msg.pose)
                        ...
                        # 这样可以直接得到hand应该到达的位置
                        target_in_hand = self.tf_listener.transformPose("hand", msg.pose)
    ...
    def search_target(self, max_time_interval=5.0, max_position_interval=0.5,rotate_step=np.radians(1),max_xyz_dist=0.3,max_yaw_dist=np.radians(0.2)):
        ...
        while len(self.queue) >= 3:
            ...
            if target1 is not None and target2 is not None and target3 is not None:
                if target1[0] > 0.5 and target2[0] > 0.5 and target3[0] > 0.5:
                    if self.xyz_distance(target1[3], target2[3]) < max_position_interval and \
                        self.xyz_distance(target2[3], target3[3]) < max_position_interval and \
                        self.xyz_distance(target1[3], target3[3]) < max_position_interval:
                        # 间距满足要求
                        if target1[0] > 0.8 and target2[0] > 0.8 and target3[0] > 0.8:
                            # 置信度满足要求
                                if abs(target1[3].header.stamp.to_sec() - target2[3].header.stamp.to_sec()) < max_time_interval and \
                                    ...
                                    # 时间间隔满足要求
                                    rospy.loginfo(f"{NODE_NAME}: 找到目标点: {target1[3]}, {target2[3]}, {target3[3]}")
                                    # 计算位置平均值：根据期望位姿
        ...
        if self.init_yaw is None:
            self.target_posestamped.pose = current_pose
        ...
        self.target_pose.pose.orientation = Quaternion(*quaternion_from_euler(0, self.pitch_offset, next_yaw))
