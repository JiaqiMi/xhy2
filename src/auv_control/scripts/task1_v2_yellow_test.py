...
"""
名称：task1_v2_yellow_test.py
功能：Task1 V2 黄色图形动作单独测试节点
描述：
    1. 启动后读取机器人当前 map -> base_link 位姿；
    2. 以当前航向为前方，先向前运动 0.5 m；
    3. 到达前方目标后保持当前位置；
    4. 发布 /auv_actuator_control，使红灯闪烁一次；
    5. 全过程目标 z 使用当前深度，不主动上浮或下潜。
监听：/tf
发布：/target，/auv_actuator_control，/finished
说明：本节点不订阅视觉话题，用固定前进距离模拟“检测到黄色图形后执行动作”。
"""
...
NODE_NAME = 'task1_v2_yellow_test'


class Task1V2YellowTest(MissionBase):
    """Task1 黄色图形动作单独测试状态机。"""
    ...
    def initialize_target(self):
        ...
        rospy.loginfo(
            '%s: yellow test target ready, target=(%.2f, %.2f)',
            NODE_NAME,
            target_x,
            target_y,
        )
        ...
        while not rospy.is_shutdown():
            ...
            # 步骤1：向前运动 0.5 m。
                ...
                if self.move_to_pose_xy(self.forward_pose):
                    rospy.loginfo('%s: reached yellow action point', NODE_NAME)
                    self.set_step(2)

            # 步骤2：黄色图形动作，红灯闪烁一次。
            elif self.step == 2:
                if self.blink_lights(red=1, green=0, count=1):
                    self.set_step(3)
