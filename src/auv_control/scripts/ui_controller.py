...
class SenderUI:
    ...
    def __init__(self, master):
        ...
        ttk.Combobox(master, textvariable=self.conn_mode, values=["TCP","串口"], width=7, state="readonly").grid(row=row, column=1, sticky="w")
        ...
        row += 1
        ttk.Label(master, text="用户IP:").grid(row=row, column=0, sticky="e")
        ttk.Entry(master, textvariable=self.tcp_ip, width=14).grid(row=row, column=1, sticky="w")
        ttk.Label(master, text="端口:").grid(row=row, column=2, sticky="e")
        ttk.Entry(master, textvariable=self.tcp_port, width=8).grid(row=row, column=3, sticky="w")
        ttk.Button(master, text="连接", command=self.toggle_user_connect, width=10).grid(row=row, column=4)
        ttk.Button(master, text="断开", command=self.toggle_user_disconnect, width=10).grid(row=row, column=5)
        row += 1

        # 调试协议TCP参数
        ttk.Label(master, text="调试IP:").grid(row=row, column=0, sticky="e")
        ttk.Entry(master, textvariable=self.debug_tcp_ip, width=14).grid(row=row, column=1, sticky="w")
        ttk.Label(master, text="调试端口:").grid(row=row, column=2, sticky="e")
        ttk.Entry(master, textvariable=self.debug_tcp_port, width=8).grid(row=row, column=3, sticky="w")
        ttk.Button(master, text="连接", command=self.toggle_debug_connect, width=10).grid(row=row, column=4)
        ttk.Button(master, text="断开", command=self.toggle_debug_disconnect, width=10).grid(row=row, column=5)
        row += 1

        # 新增：传感器端口TCP参数
        ttk.Label(master, text="传感器IP:").grid(row=row, column=0, sticky="e")
        ttk.Entry(master, textvariable=self.sensor_tcp_ip, width=14).grid(row=row, column=1, sticky="w")
        ttk.Label(master, text="传感器端口:").grid(row=row, column=2, sticky="e")
        ttk.Entry(master, textvariable=self.sensor_tcp_port, width=8).grid(row=row, column=3, sticky="w")
        ttk.Button(master, text="连接", command=self.toggle_sensor_connect, width=10).grid(row=row, column=4)
        ttk.Button(master, text="断开", command=self.toggle_sensor_disconnect, width=10).grid(row=row, column=5)
        row += 1
        ...
        ttk.Combobox(master, textvariable=self.command_type, values=list(COMMAND_TYPE.keys()), width=8).grid(row=row, column=3, sticky="w")
        ttk.Label(master, text="运动模式:").grid(row=row, column=4, sticky="e")
        ttk.Combobox(master, textvariable=self.control_mode, values=list(CONTROL_TYPE.keys()), width=8).grid(row=row, column=5, sticky="w")
        ttk.Label(master, text="控制类型:").grid(row=row, column=6, sticky="e")
        ttk.Combobox(master, textvariable=self.control_type, values=list(CONTROL_MODE.keys()), width=8).grid(row=row, column=7, sticky="w")
        row += 1

        # 空间运动控制
        ttk.Label(master, text="空间运动(6自由度):").grid(row=row, column=0, sticky="e")
        for i in range(6):
            ttk.Entry(master, textvariable=self.motion_values[i], width=4).grid(row=row, column=1+i, sticky="w")
        row += 1
        ...
        ttk.Button(master, text="生成用户报文", command=self.generate_user_packet, width=12).grid(row=row, column=6)
        ttk.Button(master, text="发送用户报文", command=self.send_user_once, width=12).grid(row=row, column=7)
        self.send_btn = ttk.Button(master, text="循环发送用户报文", command=self.toggle_user_send_loop, width=16)
        self.send_btn.grid(row=row, column=8)
        row += 1
        # 报文HEX显示
        ttk.Label(master, text="用户报文HEX:").grid(row=row, column=0, sticky="ne")
        ttk.Entry(master, textvariable=self.packet_hex, width=90).grid(row=row, column=1, columnspan=10, sticky="w")
        row += 1
        # LED与机械手
        ttk.Label(master, text="LED(绿色):").grid(row=row, column=0, sticky="e")
        ttk.Entry(master, textvariable=self.led_values[0], width=8).grid(row=row, column=1, sticky="w")
        ttk.Label(master, text="LED(红色):").grid(row=row, column=2, sticky="e")
        ttk.Entry(master, textvariable=self.led_values[1], width=8).grid(row=row, column=3, sticky="w")           
        ttk.Label(master, text="机械手:").grid(row=row, column=4, sticky="e")
        ttk.Entry(master, textvariable=self.gimbal_value, width=8).grid(row=row, column=5, sticky="w")
        row += 1
        # 补光灯控制UI
        ttk.Label(master, text="补光灯1(0-100):").grid(row=row, column=0, sticky="e")
        ttk.Entry(master, textvariable=self.light1_value, width=8).grid(row=row, column=1, sticky="w")
        ttk.Label(master, text="补光灯2(0-100):").grid(row=row, column=2, sticky="e")
        ttk.Entry(master, textvariable=self.light2_value, width=8).grid(row=row, column=3, sticky="w")
        # ========== 传感器报文UI ==========
        ttk.Button(master, text="生成传感器报文", command=self.generate_sensor_packet, width=12).grid(row=row, column=6)
        ttk.Button(master, text="发送传感器报文", command=self.send_sensor_once, width=12).grid(row=row, column=7)
        self.sensor_send_btn = ttk.Button(master, text="循环发送传感器报文", command=self.toggle_sensor_send_loop, width=16)
        self.sensor_send_btn.grid(row=row, column=8)
        row += 1

        # ========== 点控制UI ==========
        ttk.Label(master, text="点控制X(前,m):").grid(row=row, column=0, sticky="e")
        ttk.Entry(master, textvariable=self.point_x, width=8).grid(row=row, column=1, sticky="w")
        ttk.Label(master, text="Y(右,m):").grid(row=row, column=2, sticky="e")
        ttk.Entry(master, textvariable=self.point_y, width=8).grid(row=row, column=3, sticky="w")
        ttk.Label(master, text="Z(下,m):").grid(row=row, column=4, sticky="e")
        ttk.Entry(master, textvariable=self.point_z, width=8).grid(row=row, column=5, sticky="w")
        ttk.Button(master, text="生成扩展报文", command=self.generate_debug_packet, width=12).grid(row=row, column=6)
        ttk.Button(master, text="发送扩展报文", command=self.send_debug_once, width=12).grid(row=row, column=7)
        self.point_send_btn = ttk.Button(master, text="循环发送扩展报文", command=self.toggle_debug_send_loop, width=16)
        self.point_send_btn.grid(row=row, column=8)
        row += 1

        # 增加姿态偏移量UI
        ttk.Label(master, text="横滚偏移(度):").grid(row=row, column=0, sticky="e")
        self.point_roll = tk.DoubleVar(value=0.0)
        ttk.Entry(master, textvariable=self.point_roll, width=8).grid(row=row, column=1, sticky="w")
        ttk.Label(master, text="俯仰偏移(度):").grid(row=row, column=2, sticky="e")
        self.point_pitch = tk.DoubleVar(value=0.0)
        ttk.Entry(master, textvariable=self.point_pitch, width=8).grid(row=row, column=3, sticky="w")
        ttk.Label(master, text="航向偏移(度):").grid(row=row, column=4, sticky="e")
        self.point_yaw = tk.DoubleVar(value=0.0)
        ttk.Entry(master, textvariable=self.point_yaw, width=8).grid(row=row, column=5, sticky="w")
        row += 1
        ...
        self.user75_text = tk.Text(frame, width=55, height=27)
        ...
        self.debug_recv_text = tk.Text(frame, width=55, height=27)
        self.debug_recv_text.grid(row=1, column=3, sticky="w")
        # 在frame中添加传感器数据显示区域
        ttk.Label(frame, text="传感器数据:").grid(row=2, column=0, sticky="ne")
        self.sensor_text = tk.Text(frame, width=55, height=5)
        self.sensor_text.grid(row=2, column=1, sticky="w")
    ...
    def toggle_sensor_connect(self):
        ...
        try:
            ...
            # 自动启动传感器接收线程
            if not self.is_sensor_receiving:
                self.is_sensor_receiving = True
                self.sensor_recv_thread = threading.Thread(target=self.sensor_recv_loop, daemon=True)
                self.sensor_recv_thread.start()
        except Exception as e:
            self.sensor_sock = None
    ...
    def toggle_sensor_disconnect(self):
        ...
        try:
            # 先停止接收线程
            self.is_sensor_receiving = False
            if self.sensor_recv_thread:
                self.sensor_recv_thread.join(timeout=1.0)
    ...
    def validate_number(self, value, name, allow_float=True, min_val=None, max_val=None):
        ...
        try:
            ...
            # return None
            raise ValueError(f"{name}必须是有效的{'数值' if allow_float else '整数'}")
    ...
    def build_sensor_packet(self):
        ...
        # 43-44 补光灯控制
        packet[43] = self.validate_number(self.light1_value.get(), "补光灯1", allow_float=False, min_val=0, max_val=100)
        packet[44] = self.validate_number(self.light2_value.get(), "补光灯2", allow_float=False, min_val=0, max_val=100)
        # 46-49 保留
    ...
    def sensor_recv_loop(self):
        """
        传感器数据接收线程函数
        """
        buffer = bytearray()
        while self.is_sensor_receiving:
            try:
    ...
    def show_sensor_data(self, data: SensorData):
        """
        显示传感器数据
        """
        self.sensor_text.delete(1.0, tk.END)
        self.sensor_text.insert(tk.END, f"控制电压: {data.control_voltage:.2f}V\n")
        self.sensor_text.insert(tk.END, f"动力电压: {data.power_voltage:.2f}V\n")
        self.sensor_text.insert(tk.END, f"控制电流: {data.control_current:.2f}A\n")
        self.sensor_text.insert(tk.END, f"动力电流: {data.power_current:.2f}A\n")
        self.sensor_text.insert(tk.END, f"舵机角度: {data.servo_angle}°\n")
